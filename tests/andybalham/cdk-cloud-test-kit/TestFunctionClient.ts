...
import {
  ...
  ObservationTestItem,
  TestItemPrefix,
} from '@andybalham/cdk-cloud-test-kit/TestItems';
import TestObservation from '@andybalham/cdk-cloud-test-kit/TestObservation';
import { TestProps } from '@andybalham/cdk-cloud-test-kit/TestProps';
...
export default class TestFunctionClient {
  ...
  async getTestPropsAsync(): Promise<TestProps> {
    ...
    const testQueryOutput = await documentClient
  ...
  async getFunctionStateAsync(
    ...
    const functionStateQueryOutput = await documentClient
